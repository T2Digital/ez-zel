...
import { Home, Search, X, Brain, Activity, AlertTriangle, Cloud, RefreshCw, CloudOff, Ear, DollarSign, Shield, Waves, Key, Smartphone, Volume2, VolumeX } from 'lucide-react';
...
export const TopNavigation: React.FC<TopNavigationProps> = ({
    onBack, isSearchActive, setIsSearchActive, searchQuery, setSearchQuery, searchInputRef,
                ...
                {!isSearchActive && (
                    <>
                        ...
                        {onOpenAffiliate && !isRestrictedMode && (
                            <button onClick={onOpenAffiliate} className="p-2 bg-emerald-900/20 border border-emerald-500/20 rounded-full text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all">
                                <DollarSign className="w-4 h-4" />
                            </button>
                        )}
                        <button onClick={() => setIsSearchActive(true)} className="p-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 text-white/50 hover:text-white transition-all">
                        ...
                        <button onClick={() => setShowLiveAPIMode(true)} className="p-2 rounded-full bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/20 text-blue-400 transition-all select-none" title="Live API (محاكاة WebRTC)">
                            <Waves className="w-4 h-4" />
                        </button>
                        <button onClick={() => setShowMemoryVault(true)} className="p-2 rounded-full bg-fuchsia-500/10 hover:bg-fuchsia-500/20 border border-fuchsia-500/20 text-fuchsia-400 transition-all select-none" title="الذاكرة المعرفية">
                        ...
                        <button onClick={() => setShowNativeSettings(true)} className="p-2 rounded-full bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/20 text-cyan-400 transition-all select-none hidden md:block" title="إعدادات النظام العميق">
                            <Smartphone className="w-4 h-4" />
                        </button>
                        <button onClick={() => setIsMuted(!isMuted)} className={`p-2 rounded-full border transition-all ${isMuted ? 'bg-white/5 border-white/10 text-white/30' : 'bg-cyan-500/10 border-cyan-500/50 text-cyan-400'}`}>
