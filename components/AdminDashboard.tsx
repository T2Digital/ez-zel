...
const AdminDashboard: React.FC<Props> = ({ onLogout, onSwitchToUserMode }) => {
  ...
  useEffect(() => { 
      const initData = async () => {
          // 1. Load Admin Profile (To get correct Referral Code)
          const titoProfile = await shadowDB.getProfile('TITO');
          if (titoProfile) {
              // MERGE: Ensure affiliate data exists even if DB profile is old
              const mergedProfile = { 
                  ...adminProfile, 
                  ...titoProfile, 
                  affiliate: titoProfile.affiliate || adminProfile.affiliate 
              };
              setAdminProfile(mergedProfile);
              
              // Self-Heal: Update DB if affiliate was missing
              if (!titoProfile.affiliate) {
                  await shadowDB.saveProfile(mergedProfile);
              }
          } else {
              // Create if missing
  ...
  const handleStatusUpdate = async (phone: string, status: 'active' | 'blocked' | 'pending') => {
    ...
    if (profile) {
        if (status === 'active' && profile.status !== 'active' && profile.referredBy && !profile.commissionPaid) {
            // FIX: Dynamic Commission based on Cycle
  ...
  const confirmPayout = async () => {
      ...
      // Ensure Payout is Recorded in History & deducted from balance
  ...
  // Microphone Logic for Core Rules
  ...
  // Microphone Logic for Broadcast
  ...
  const handleSendBroadcast = async () => { 
      ...
      try { 
          // New "Pulse" method using Config instead of direct loop
          await shadowDB.setGlobalPulse(broadcastMessage);
          setBroadcastMessage(''); 
          alert("تم إطلاق نبض الظل. سيصل لجميع المستخدمين عند فتحهم للتطبيق."); 
      } catch (e) { 
          console.error(e); 
  ...
  // Members (Active Subscribers, NOT just marketers)
  ...
  const totalRevenue = activeMembers.reduce((sum, p) => {
      // Calculate based on their specific plan cycle
      const planPrice = p.subscriptionCycle === 'yearly' ? 10000 : 1000;
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white flex flex-col font-['Cairo'] relative pb-48 overflow-x-hidden">
      ...
      <div className="p-6 md:p-8 flex justify-between items-center bg-black/50 backdrop-blur-md border-b border-white/5 sticky top-0 z-50">
          ...
          <div className="flex items-center gap-2">
              <button 
                ...
                className="px-4 py-2 bg-white/5 border border-white/10 hover:bg-white/10 rounded-full text-white flex items-center gap-2 transition-all group"
                title="فتح واجهة المستخدم (الظل، نكسوس، التسويق)"
              >
                  <Smartphone className="w-4 h-4 text-emerald-400 group-hover:animate-pulse" />
                  <span className="text-xs font-bold">وضع المستخدم</span>
              </button>

              <button onClick={onLogout} className="p-2 hover:bg-white/10 rounded-full text-red-500/50 hover:text-red-500 transition-all">
        ...
        {activeView === 'broadcast' && (
             <div className="animate-in fade-in zoom-in duration-500 h-full flex flex-col">
                 <div className="flex items-center justify-between mb-6">
                     <div className="flex items-center gap-3">
                         <div className="p-3 bg-amber-500 rounded-full shadow-[0_0_30px_rgba(245,158,11,0.4)] animate-pulse">
                             <Megaphone className="w-6 h-6 text-black" />
                         </div>
                         <div>
                             <h2 className="text-2xl font-black text-white">نبض الظل (Shadow Pulse)</h2>
                             <p className="text-[10px] text-amber-500 font-bold uppercase tracking-[0.2em]">Global System Broadcast</p>
                         </div>
                     </div>
                     <div className="bg-red-500/10 border border-red-500/20 px-3 py-1 rounded-full">
                        <span className="text-[10px] text-red-400 font-black animate-pulse">● LIVE TO ALL USERS</span>
                     </div>
                 </div>
                 ...
                 <div className="flex-1 bg-[#111] border border-amber-500/20 rounded-[32px] p-8 relative overflow-hidden flex flex-col shadow-2xl">
                     ...
                     <div className="mb-4">
                        <label className="text-[10px] text-white/30 font-bold uppercase tracking-widest mb-2 block">محتوى النبضة (تظهر كتنبيه سيستم للجميع)</label>
                        <textarea 
                            value={broadcastMessage} 
                            onChange={(e) => setBroadcastMessage(e.target.value)} 
                            className="w-full h-40 bg-black border border-white/10 rounded-2xl p-4 text-white text-lg leading-relaxed resize-none focus:border-amber-500/50 outline-none transition-all placeholder:text-white/20" 
                            placeholder="اكتب رسالتك هنا.. (تهنئة، تحديث هام، تحذير، أو نصيحة عامة)..." 
                        />
                     </div>

                     <div className="mt-auto flex items-center justify-between border-t border-white/5 pt-6">
                        <div className="flex items-center gap-3">
                            <button onClick={handleBroadcastMic} className={`p-4 rounded-full transition-all border ${isListeningBroadcast ? 'bg-amber-500 text-black border-amber-600 animate-pulse' : 'bg-white/5 border-white/10 text-white/40 hover:text-white'}`}>
                                <Mic className="w-6 h-6" />
                            </button>
                            <span className="text-[10px] text-white/30 max-w-[150px] hidden md:block">
                                * سيتم إرسال هذا النبض لجميع المشتركين والمسوقين وتيتو فوراً.
                            </span>
                        </div>
                        ...
                        <button 
                            ...
                            className="px-8 py-4 bg-amber-500 hover:bg-amber-400 text-black rounded-2xl font-black flex items-center gap-3 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 shadow-[0_0_30px_rgba(245,158,11,0.3)]"
                        >
                            {isBroadcasting ? <Activity className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
                            {isBroadcasting ? 'جاري البث...' : 'إرسال النبض للجميع'}
                        </button>
                     </div>
                 </div>
             </div>
                ...
                {feedbacks.length > 0 ? feedbacks.map((fb, idx) => (
                    <div key={idx} className="bg-[#080808] p-5 rounded-[24px] border border-white/5 flex flex-col gap-2 relative group hover:bg-white/5 transition-all">
                        <div className="flex justify-between items-start">
                            <div>
                                <h4 className="font-bold text-white text-sm">{fb.userName} <span className="text-xs text-white/30 font-mono">({fb.userId})</span></h4>
                                <span className="text-[10px] text-white/20">{new Date(fb.timestamp).toLocaleString('ar-EG')}</span>
                            </div>
                            {!fb.isRead && <span className="w-2 h-2 rounded-full bg-purple-500 animate-pulse"></span>}
                        </div>
                        <div className="bg-black/30 p-3 rounded-xl text-sm text-white/80 leading-relaxed border border-white/5">
                            "{fb.message}"
                        </div>
                    </div>
                )) : (
            ...
            <>
                ...
                {activeMembers.map(u => (
                    <div key={u.phone} className="bg-[#080808] p-4 rounded-2xl border border-white/5 flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                            <div className={`w-2 h-12 rounded-full ${u.status === 'active' ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                            <div>
                                <h4 className="font-bold text-sm text-white flex items-center gap-2">
                                    {u.name}
                                </h4>
                                <div className="flex items-center gap-2 mt-1"><span className="text-[10px] bg-white/5 px-2 py-0.5 rounded text-white/40 font-mono">{u.phone}</span>{u.subscriptionCycle && <span className="text-[9px] text-amber-500 bg-amber-900/20 px-1.5 py-0.5 rounded">{u.subscriptionCycle === 'yearly' ? 'سنوي' : 'شهري'}</span>}</div>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => handleStatusUpdate(u.phone, u.status === 'active' ? 'blocked' : 'active')} className="p-2 hover:bg-white/10 rounded-full transition-colors text-white/30 hover:text-white">{u.status === 'active' ? <ShieldCheck className="w-5 h-5 text-emerald-500" /> : <XCircle className="w-5 h-5 text-red-500" />}</button>
                        </div>
                    </div>
                ))}
            </>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="text-[10px] bg-white/5 px-2 py-0.5 rounded text-white/40 font-mono">{u.phone}</span>
                                    <span className="text-[9px] bg-emerald-900/20 text-emerald-400 px-1.5 py-0.5 rounded font-mono font-bold">رصيد: {u.affiliate?.totalEarnings}ج</span>
                                </div>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <button onClick={() => openPayoutModal(u)} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-purple-900/20">
                                <DollarSign className="w-4 h-4" /> صرف
                            </button>
                            <button onClick={() => handleStatusUpdate(u.phone, u.status === 'active' ? 'blocked' : 'active')} className="p-2 hover:bg-white/10 rounded-full transition-colors text-white/30 hover:text-white">{u.status === 'active' ? <CheckCircle className="w-5 h-5 text-emerald-500" /> : <XCircle className="w-5 h-5 text-red-500" />}</button>
                        </div>
                    </div>
                ))}
            </>
      ...
      {/* --- MASTER BOTTOM NAVIGATION (Separated & Symmetrical) --- */}
      <nav className="fixed bottom-[32px] left-0 w-full bg-black/95 backdrop-blur-xl border-t border-white/10 pb-6 pt-3 px-2 z-[100] rounded-t-[40px] shadow-[0_-10px_40px_rgba(0,0,0,0.8)]">
        <div className="flex items-end justify-between max-w-lg mx-auto w-full relative px-2">
             ...
             {/* Left Wing: Members, Marketers, Requests */}
             <div className="flex gap-1 items-end">
                <button onClick={() => setActiveView('members')} className={`flex flex-col items-center gap-1 p-2 group transition-all w-14 ${activeView === 'members' ? 'text-white' : 'text-white/30'}`}>
                    <Users className="w-5 h-5 group-hover:text-white" />
                    <span className="text-[8px] font-bold tracking-wider">الأعضاء</span>
                </button>

                <button onClick={() => setActiveView('marketers')} className={`flex flex-col items-center gap-1 p-2 group transition-all w-14 ${activeView === 'marketers' ? 'text-white' : 'text-white/30'}`}>
                    <Briefcase className="w-5 h-5 group-hover:text-white" />
                    <span className="text-[8px] font-bold tracking-wider">المسوقين</span>
                </button>

                <button onClick={() => setActiveView('requests')} className={`flex flex-col items-center gap-1 p-2 group transition-all w-14 relative ${activeView === 'requests' ? 'text-white' : 'text-white/30'}`}>
                    <div className="relative">
                        <CreditCard className="w-5 h-5 group-hover:text-white" />
                        {pendingRequests.length > 0 && <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full animate-bounce"></span>}
                    </div>
                    <span className="text-[8px] font-bold tracking-wider">الطلبات</span>
                </button>
             </div>
            ...
            {/* Right Wing: Feedback, Broadcast, Core */}
            <div className="flex gap-1 items-end">
                <button onClick={() => setActiveView('feedback')} className={`flex flex-col items-center gap-1 p-2 group transition-all w-14 relative ${activeView === 'feedback' ? 'text-white' : 'text-white/30'}`}>
                    <div className="relative">
                        <MessageSquare className="w-5 h-5 group-hover:text-white" />
                        {feedbacks.some(f => !f.isRead) && <span className="absolute -top-1 -right-1 w-2 h-2 bg-purple-500 rounded-full animate-pulse"></span>}
                    </div>
                    <span className="text-[8px] font-bold tracking-wider">الآراء</span>
                </button>

                <button onClick={() => setActiveView('broadcast')} className={`flex flex-col items-center gap-1 p-2 group transition-all w-14 ${activeView === 'broadcast' ? 'text-white' : 'text-white/30'}`}>
                    <Megaphone className="w-5 h-5 group-hover:text-white" />
                    <span className="text-[8px] font-bold tracking-wider">النبض</span>
                </button>

                <button onClick={() => setActiveView('core')} className={`flex flex-col items-center gap-1 p-2 group transition-all w-14 ${activeView === 'core' ? 'text-white' : 'text-white/30'}`}>
                    <Cpu className="w-5 h-5 group-hover:text-red-500 transition-colors" />
                    <span className="text-[8px] font-bold tracking-wider">النواة</span>
                </button>
            </div>
      ...
      {selectedProof && (
          <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in" onClick={() => setSelectedProof(null)}>
              <img src={selectedProof} className="max-w-full max-h-[90vh] rounded-[24px] border border-white/20 shadow-2xl" alt="Proof" />
              <button className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-red-600 rounded-full text-white transition-all"><X className="w-6 h-6" /></button>
          </div>
      )}
              ...
              <div className="max-w-md w-full bg-[#111] border border-emerald-500/30 rounded-[32px] p-6 relative shadow-[0_0_50px_rgba(16,185,129,0.2)]">
                  ...
                  <div className="space-y-4">
                       <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                           <p className="text-[10px] text-white/40 uppercase font-bold mb-1">المسوق</p>
                           <p className="font-bold text-white mb-2">{payoutModal.user.name}</p>
                           {/* Show Saved Payout Details */}
                           {payoutModal.user.affiliate?.payoutDetails?.number && (
                               <div className="mt-2 pt-2 border-t border-white/5">
                                   <p className="text-[10px] text-emerald-400 font-bold">بيانات السحب المسجلة:</p>
                                   <p className="text-xs text-white font-mono">{payoutModal.user.affiliate.payoutDetails.method.toUpperCase()}: {payoutModal.user.affiliate.payoutDetails.number}</p>
                                   <p className="text-[10px] text-white/50">الاسم: {payoutModal.user.affiliate.payoutDetails.name}</p>
                               </div>
                           )}
                           {/* Show Withdrawal History in Modal */}
                           {payoutModal.user.affiliate?.payoutHistory && payoutModal.user.affiliate.payoutHistory.length > 0 && (
                               <div className="mt-4 pt-4 border-t border-white/5 max-h-32 overflow-y-auto scrollbar-hide">
                                   <h5 className="text-[10px] text-white/50 uppercase font-bold mb-2 flex items-center gap-1"><Activity className="w-3 h-3" /> سجل المعاملات السابقة</h5>
                                   {payoutModal.user.affiliate.payoutHistory.map((h, idx) => (
                                       <div key={idx} className="flex justify-between items-center text-xs text-white/80 mb-2 bg-black/40 p-2 rounded-lg">
                                           <span className="font-mono">{new Date(h.date).toLocaleDateString()}</span>
                                           <span className="text-emerald-400 font-bold">{h.amount}ج</span>
                                       </div>
                                   ))}
                               </div>
                           )}
      ...
      {/* AGENT DETAILS MODAL (Restored) */}
      {selectedAgent && (
          <div className="fixed inset-0 z-[250] bg-black/95 backdrop-blur-md p-6 flex items-center justify-center animate-in zoom-in">
              <div className="max-w-sm w-full bg-[#111] border border-white/10 rounded-[32px] p-8 relative shadow-2xl">
                  <button onClick={() => setSelectedAgent(null)} className="absolute top-4 left-4 text-white/30 hover:text-white"><X className="w-5 h-5" /></button>
                  <div className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-6 bg-${selectedAgent.color}-500/10 border border-${selectedAgent.color}-500/20 text-${selectedAgent.color}-400 shadow-[0_0_30px_rgba(0,0,0,0.3)]`}>
                      {React.cloneElement(selectedAgent.icon, { className: "w-10 h-10" })}
                  </div>
                  <h3 className="text-2xl font-black text-white text-center mb-1">{selectedAgent.role}</h3>
                  <p className="text-center text-[10px] text-white/40 uppercase font-bold tracking-widest mb-6">{selectedAgent.name} • {selectedAgent.status}</p>
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                      <p className="text-sm text-white/80 leading-relaxed text-center">{selectedAgent.description}</p>
                  </div>
              </div>
          </div>
      )}
