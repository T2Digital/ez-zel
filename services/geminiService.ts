...
const getFallbackResponse = (input: string): string => {
    return "نظام الظل بيعمل إعادة تحميل (Rebooting)... ثواني وراجعلك أقوى.";
};
...
const callGeminiWithRetry = async (params: any, retries = 3, delay = 2000): Promise<GenerateContentResponse> => {
    try {
        ...
        if ((isQuotaError || isOverloaded) && retries > 0) {
            console.warn(`[Shadow Core] Gemini 3 Busy (${error.status}). Retrying in ${delay}ms...`);
            const backoff = delay * 1.5 + Math.random() * 500; // Add jitter
...
export const getShadowResponse = async (
    ...
    try {
        ...
        // Clean History for Gemini 3 Compatibility
        const validHistory = history
            .filter(m => m.parts && m.parts[0] && m.parts[0].text)
            .map(m => ({ role: m.role, parts: [{ text: m.parts[0].text }] })) // Remove complex parts from history to save tokens/errors
            .slice(-10); 
